...
public actor HistogramCalculator {
    ...
    private func getCIImage(histogramArray: [UInt32], writeToDisk: Bool, binCount: Int) -> CIImage? {
        let max = histogramArray.max()!
        
        guard let bitmapData = createBitmapDataFromArray(
            colorArray: histogramArray,
            maxCount: max,
            binCount: binCount
        ) else {
            return nil
        ...
        let image = CIImage(
            ...
            bytesPerRow: bitmapData.count,
            size: CGSize(width: binCount, height: 1),
            format: .RGBA8,
            colorSpace: nil
        )
        ...
        do {
            if writeToDisk {
                let tempFile = try FileManager.default.url(
                    for: .documentDirectory,
                    in: .userDomainMask,
                    appropriateFor: nil,
                    create: true
                ).appendingPathComponent("hist", conformingTo: .heif)
            ...
            LibLogger.defaultLogger.error("Failed to save histogram image: \(error)")
            return nil
        }
    }
    
    private func createBitmapDataFromArray(
        colorArray: HistogramInfo,
        maxCount: UInt32,
        binCount: Int
    ) -> Data? {
        // Ensure the array has the expected size
        guard colorArray.count == binCount * 3 else {
            LibLogger.defaultLogger.error("Invalid array size. Expected 768 (256 * 3).")
            return nil
        }
        
        var bitmapData = Data(capacity: binCount * 4) // 256 pixels * 4 bytes (RGBA)
        
        for x in 0..<binCount {
            var r = colorArray[x] // Red channel
            var g = colorArray[x + binCount] // Green channel
            var b = colorArray[x + binCount * 2] // Blue channel
            
            let rProgress = Float(r) / Float(maxCount)
            r = UInt32(rProgress * 255)
            
            let gProgress = Float(g) / Float(maxCount)
            g = UInt32(gProgress * 255)
            
            let bProgress = Float(b) / Float(maxCount)
            b = UInt32(bProgress * 255)
            
            let a: UInt8 = 255 // Alpha channel (fully opaque)
            
            // Append the RGBA bytes to the Data object
            bitmapData.append(UInt8(r))
            bitmapData.append(UInt8(g))
            bitmapData.append(UInt8(b))
            bitmapData.append(a)
        }
    ...
    /// Calculate and return the maximum `CGColor` of a `CIImage` when applied with the gain map.
    ///
    /// - parameter ciImage: The CIImage to calculate the maximum pixel value of.
    /// - parameter gainMapImage: The gain map CIImage.
    /// - parameter maxHeadroom: The maximum headroom value.
    private func calculateMaxPixelValue(
        ciImage: CIImage,
        ...
        let imageMaxColor = try await calculateMaxPixelValue(
            ciImage: ciImage,
            targetColorSpace: targetColorSpace,
            maxSizeInPixel: maxSizeInPixel
        )
        ...
        let gainMapMaxColor = try await calculateMaxPixelValue(
            ciImage: gainMapImage,
            targetColorSpace: targetColorSpace,
            maxSizeInPixel: maxSizeInPixel
        )
        ...
        guard let imageMaxComponent = imageMaxColor.components?.suffix(3).max() else {
            throw IllegalStateException(message: "Failed to get max component")
        }
        guard let gainMapMaxComponent = gainMapMaxColor.components?.suffix(3).max() else {
            throw IllegalStateException(message: "Failed to get max component")
        }
        
        let maxPixelValue = imageMaxComponent * (1.0 + (maxHeadroom - 1.0) * gainMapMaxComponent)
        return maxPixelValue
    }
    
    /// Calculate and return the maximum `CGColor` of a `CIImage`.
    ///
    /// To get the RGBA components of the color, use the `components` property of the `CGColor`.
    ///
    /// - parameter ciImage: The CIImage to calculate the maximum pixel value of.
    private func calculateMaxPixelValue(
        ciImage: CIImage,
        targetColorSpace: CGColorSpace,
        maxSizeInPixel: CGFloat
    ) async throws -> (CGColor) {
        let ciImage = ciImage
        
        guard let commandQueue = device.makeCommandQueue() else {
            LibLogger.defaultLogger.error("Failed to create Metal command queue")
            throw IllegalStateException(message: "failed to create metal command queue")
        }
        ...
        guard let sourceTexture = createMetalTexture(
            ...
            LibLogger.defaultLogger.error("Failed to create Metal texture")
            throw IllegalStateException(message: "Failed to create metal texture")
        }
        
        try Task.checkCancellation()
        
        guard let imageMinMaxTexture = create2x1MetalTexture(
        ...
        guard let commandBuffer = commandQueue.makeCommandBuffer() else {
            LibLogger.defaultLogger.error("Failed to create Metal command buffer")
            throw IllegalStateException(message: "Failed to create metal command buffer")
        }
        
        try Task.checkCancellation()
        
        let calculation = MPSImageStatisticsMinAndMax(device: device)
        calculation.encode(
            commandBuffer: commandBuffer,
            sourceTexture: sourceTexture,
            destinationTexture: imageMinMaxTexture
        )
        ...
        guard let bytes = get2x1TextureBytes(texture: imageMinMaxTexture), bytes.count == 8 else {
            throw IllegalStateException(message: "Failed to get texture bytes")
        }
        
        let r: CGFloat = CGFloat(bytes[4]) / 255.0
        let g: CGFloat = CGFloat(bytes[5]) / 255.0
        let b: CGFloat = CGFloat(bytes[6]) / 255.0
        let a: CGFloat = CGFloat(bytes[7]) / 255.0
